...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    return SparseVector(numOHEFeats, sorted(OHEDict[f] for f in rawFeats), [1 for f in rawFeats])
...
IFrame("http://labs.criteo.com/2014/02/download-kaggle-display-advertising-challenge-dataset/",
       600, 350)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    return SparseVector(numOHEFeats, 
                        sorted([OHEDict[f] for f in rawFeats if f in OHEDict]), 
...
def parseHashPoint(point, numBuckets):
    ...
    items = point.split(',')
    label = items[0]
    features = hashFunction(numBuckets, parsedPoint)
...
def computeSparsity(data, d, n):
    ...
    """
    return float(data.map(lambda x: len(x.features.indices)).reduce(lambda x, y: x+y) ) / (d*n)
