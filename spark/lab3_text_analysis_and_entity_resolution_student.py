...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, lambda x: -1 * x[1])
...
def tf(tokens):
    ...
    for token in tokens:
        if token in res:
            ...
            res[token] = 1
    for ky in res:
        res[ky] = res[ky] /float(len(tokens))
    return res
...
#     print(record)
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast
             .value
             .get(amazonRec, {})
             .get(t, 0) * googleWeightsBroadcast.value.get(googleRec, {}).get(t, 0) 
             for t in tokens])
